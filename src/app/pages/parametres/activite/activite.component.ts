...
import {ActiviteDialogComponent} from './activite-dialog/activite-dialog.component';
...
export class ActiviteComponent {
  ...
  constructor(public appSettings:AppSettings, public http: HttpClient, public dialog: MatDialog, public router: Router,
    ...
    this.settings = this.appSettings.settings;
      this.getListeActivites().subscribe(data => {
        this.temp = [...data];
        this.rows = data;
    ...
    getListeActivites(): Observable<Entite[]> {
        return this.http.get<Entite[]>(this.appSettings.rootUrl + 'api/les_activites');
    }
  ...
  onActivate(event) {
    ...
    deleteActivite(activite: any) {
        this.msg = 'Êtes-vous sûr de vouloir supprimer cette activité?';
    ...
    delete(activite){
        ...
        this.controleService.deleteActivite(activite.id).subscribe((result: any) =>{
                this.temp = [];
    ...
    public openActiviteDialog(activite) {
        let dialogRef = this.dialog.open(ActiviteDialogComponent, {
            data: activite
        });
